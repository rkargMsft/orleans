...
internal sealed class ResourceOptimizedPlacementLogic : ISiloStatisticsChangeListener
{
    ...
    public ResourceOptimizedPlacementLogic(
    ...
    public Task<SiloAddress> PickSilo(IPlacementContext context, Span<SiloAddress> candidateSilos)
    {
        if (candidateSilos.Length == 0)
        {
            throw new SiloUnavailableException($"Cannot place grain because there are no compatible silos.");
        }

        if (candidateSilos.Length == 1)
        {
            return Task.FromResult(candidateSilos[0]);
        }

        if (_siloStatistics.IsEmpty)
        {
            return Task.FromResult(candidateSilos[Random.Shared.Next(candidateSilos.Length)]);
        }
        ...
        int compatibleSilosCount = candidateSilos.Length;
        if (compatibleSilosCount * Unsafe.SizeOf<(int, ResourceStatistics)>() <= FourKiloByte)
        {
            pick = MakePick(stackalloc (int, ResourceStatistics)[compatibleSilosCount], candidateSilos);
        }
        else
        {
            var relevantSilos = ArrayPool<(int, ResourceStatistics)>.Shared.Rent(compatibleSilosCount);
            pick = MakePick(relevantSilos.AsSpan(), candidateSilos);
        ...
        {
            var bestCandidate = candidateSilos[pick.Index];
            return Task.FromResult(bestCandidate);
        }

        return _cachedLocalSilo;

        (int PickIndex, float PickScore, float? LocalSiloScore) MakePick(scoped Span<(int, ResourceStatistics)> relevantSilos, Span<SiloAddress> candidateSilos)
        {
            ...
            for (var i = 0; i < candidateSilos.Length; ++i)
            {
                var silo = candidateSilos[i];
